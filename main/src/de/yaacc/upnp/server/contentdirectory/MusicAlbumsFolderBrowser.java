...
public class MusicAlbumsFolderBrowser extends ContentBrowser {
	...
	public DIDLObject browseMeta(YaaccContentDirectory contentDirectory, String myId) {
		...
		StorageFolder folder = new StorageFolder(ContentDirectoryIDs.MUSIC_ALBUMS_FOLDER.getId(), ContentDirectoryIDs.MUSIC_FOLDER.getId(), "Genres", "yaacc", getSize(contentDirectory,myId),
				907000L);
		return folder;
	}
	...
	public List<Container> browseContainer(YaaccContentDirectory contentDirectory, String myId) {
		...
		if (mediaCursor != null) {
			...
			while (!mediaCursor.isAfterLast()) {
				...
				MusicAlbum musicAlbum = new MusicAlbum(ContentDirectoryIDs.MUSIC_ALBUM_TRACK_PREFIX.getId()+id, ContentDirectoryIDs.MUSIC_ALBUMS_FOLDER.getId(), name, "", getMusicTrackSize(contentDirectory, id));
