...
/**
 * This class creates the all music titles folder out of the content in the media store
 * @author Tobias Schoene (openbit)
 *
 */
public class MusicAllTitlesFolderCreator  {

	public Container build(YaaccContentDirectory contentDirectory, String parentID) {
		List<MusicTrack> musicTracks = createMediaStoreAllMusicTracks(contentDirectory, ContentDirectoryFolder.MUSIC_ALL_TITLES.getId());
		MusicAlbum allMusicAlbum = new MusicAlbum(ContentDirectoryFolder.MUSIC_ALL_TITLES.getId(), parentID, "All", "yaacc", musicTracks.size(), musicTracks);
		contentDirectory.addContent(allMusicAlbum.getId(), allMusicAlbum);
		return allMusicAlbum;
	}

	
	
	private List<MusicTrack> createMediaStoreAllMusicTracks(YaaccContentDirectory contentDirectory, String parentID) {
